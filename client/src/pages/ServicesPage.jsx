...
const SERVICES_DATA = [
  ...
  },
  {
    ...
    subheadline: 'Equipping government teams with the mental resilience and emotional stability needed to serve communities effectively under pressure.',
    cta: 'Partner With Us',
    challenge: 'Public servants operate in high-stress, unpredictable environments. Constant pressure and critical decision-making can lead to chronic stress, impacting their wellbeing and the quality of public service.',
    approach: 'We provide practical, evidence-based wellbeing programs tailored for the public sector, helping teams build resilience, manage stress, and maintain focus in demanding roles.',
    offerings: [
...
export default function ServicesPage() {
  ...
  const [dynamicServicesData, setDynamicServicesData] = useState(SERVICES_DATA)
  const [activeTab, setActiveTab] = useState(SERVICES_DATA[0].id)

  useEffect(() => {
    const fetchServices = async () => {
      try {
        const [resCards, ...contentResArray] = await Promise.all([
          axios.get(`${API}/api/sections/services?t=${Date.now()}`),
          axios.get(`${API}/api/content/corporate`).catch(() => ({ data: [] })),
          axios.get(`${API}/api/content/education`).catch(() => ({ data: [] })),
          axios.get(`${API}/api/content/community`).catch(() => ({ data: [] })),
          axios.get(`${API}/api/content/government`).catch(() => ({ data: [] }))
        ])
        
        const contentMap = {}
        contentResArray.forEach((r, i) => {
           const sectionName = ['corporate', 'education', 'community', 'government'][i]
           const items = r.data.items || r.data || []
           contentMap[sectionName] = {}
           items.forEach(it => { contentMap[sectionName][it.key] = it.value })
        })

        const dbCards = resCards.data.items || []
        if (dbCards.length > 0) {
          setDynamicServicesData(prev => prev.map(card => {
             const dbMatch = dbCards.find(d => (d.typeSlug || d.type) === card.id)
             const leg = contentMap[card.id] || {}
             if (dbMatch) {
                let pre = dbMatch.headline || card.headlinePre
                let acc = ''
                if (pre) {
                ...
                return {
                   ...card,
                   tag: dbMatch.title || card.tag,
                   headlinePre: pre,
                   headlineAccent: acc,
                   subheadline: dbMatch.description || card.subheadline,
                   image: dbMatch.image || card.image,
                   challenge: leg.challenge || card.challenge,
                   approach: leg.approach || card.approach,
                   offerings: leg.offerings ? leg.offerings.split('\n').map(s=>s.trim()).filter(Boolean) : card.offerings,
                   formats: leg.formats ? leg.formats.split('\n').map(s=>s.trim()).filter(Boolean) : card.formats,
                   outcomes: leg.outcomes ? leg.outcomes.split('\n').map(s=>s.trim()).filter(Boolean) : card.outcomes,
                }
             }
             return {
                ...card,
        ...
        } else if (Object.keys(contentMap).length > 0) {
          setDynamicServicesData(prev => prev.map(card => {
             const leg = contentMap[card.id] || {}
             return {
                ...card,
  ...
  useEffect(() => {
    if (serviceId && dynamicServicesData.find(s => s.id === serviceId)) {
      setActiveTab(serviceId)
    }
    setTimeout(() => {
      window.scrollTo({ top: 0, behavior: 'smooth' })
    }, 100)
  }, [serviceId, dynamicServicesData])

  const activeServiceData = dynamicServicesData.find(s => s.id === activeTab) || dynamicServicesData[0]
          ...
          <div style={{ position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {dynamicServicesData.filter(s => s.id !== activeTab).map((service) => (
                  <button
                    key={service.id}
                    onClick={() => {
